...
function set_empty!(conf :: C, i::Int64, j::Int64) where {BA<:BlockArray, C <: CheckConfiguration{PBC,BA}}
    # check which type of field was there before
    if is_empty(conf, i,j)
        # do nothing
    elseif is_entry(conf, i,j)
        ...
        #for x in max(i-conf.dmax,1):min(i+conf.dmax,sizex(conf))
        #for y in max((j-conf.dmax)+(abs(i-x)),1):min((j+conf.dmax)-(abs(i-x)),sizey(conf))
        for dx in -conf.dmax:conf.dmax
        for dy in -conf.dmax + abs(dx):conf.dmax - abs(dx)
            d = abs(dx) + abs(dy)
            x = i + dx
            y = j + dy
            x = bring_to_range_one_to_a(x, sizex(conf))
            y = bring_to_range_one_to_a(y, sizey(conf))
            if is_check(conf, x,y)
                if !(x==i && y==j)
                    c2 = get_check(conf,x,y)
                    conf.neighbor_pairs[min(c1,c2),max(c1,c2)][d] -= 1
...
function set_check!(conf :: C, i::Int64, j::Int64, c::Int64) where {BA<:BlockArray, C <: CheckConfiguration{PBC,BA}}
    # check which type of field was there before
    if is_empty(conf, i,j)
        # find out block indizes
        bi = get_block_index_x(conf, i)
    ...
    elseif is_entry(conf, i,j)
        ...
        for dy in -conf.dmax + abs(dx):conf.dmax - abs(dx)
            ...
            if is_check(conf, x,y)
                if !(x==i && y==j)
                    #println(i, ",", j, " --> ", x,",",y," (d=$(d), dx=$(dx), dy=$(dy))")
                    c2 = get_check(conf,x,y)
                    conf.neighbor_pairs[min(c,c2),max(c,c2)][d] += 1
                end
            end
